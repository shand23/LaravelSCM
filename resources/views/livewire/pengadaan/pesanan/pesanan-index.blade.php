<div>
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Daftar Pesanan (RFQ)</h1>
            <p class="text-sm text-gray-500 mt-1">Kelola Request for Quotation ke Supplier</p>
        </div>
    ...
    <div class="bg-white p-4 mb-6 rounded-xl shadow-sm border border-gray-100">
        <div class="w-full md:w-1/3 relative">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" 
                   placeholder="Cari Nomor Pesanan/RFQ..." 
                ...
                @forelse ($listPesanan as $psn)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center text-sm font-medium flex justify-center gap-2">
                            ...
                            {{-- Tombol Cetak PDF --}}
                            <button wire:click="cetakPDF('{{ $psn->id_pesanan }}')" class="text-blue-600 hover:text-blue-900 bg-blue-50 px-3 py-1.5 rounded-md border border-blue-100 flex items-center gap-1" title="Cetak / Download RFQ">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                                Cetak PDF
                            </button>
                            
                            {{-- Edit & Delete MUNCUL SELAMA status BUKAN 'Berlanjut ke Kontrak' --}}
            ...
            <div class="p-6 overflow-y-auto bg-white flex-1">
                <form wire:submit.prevent="store">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8 bg-blue-50/50 p-5 rounded-xl border border-blue-100">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Pilih PR (Logistik)</label>
                            <select wire:model.live="id_pengajuan" class="w-full border-gray-300 rounded-lg shadow-sm border px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500 {{ $isEditMode ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-white' }}" {{ $isEditMode ? 'disabled' : '' }}>
                                <option value="">-- Pilih PR --</option>
                                @foreach($listPR as $pr)
                                    <option value="{{ $pr->id_pengajuan }}">{{ $pr->id_pengajuan }} ({{ $pr->permintaanProyek->proyek->nama_proyek ?? 'Restok' }})</option>
                                @endforeach
                            </select>
                            @error('id_pengajuan') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                    ...
                    @if(empty($items))
                        <div class="text-center py-12 text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
                            Silakan pilih Referensi PR di atas terlebih dahulu untuk memuat daftar material.
                        </div>
                    @else
                        @error('items') 
                            <div class="bg-red-50 text-red-600 px-4 py-2 rounded-lg text-sm mb-4 border border-red-100">{{ $message }}</div>
                        @enderror
