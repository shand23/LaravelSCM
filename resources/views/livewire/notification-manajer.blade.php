<header wire:poll.3s class="bg-white shadow-sm px-6 py-3 flex justify-between items-center sticky top-0 z-20">
    ...
    <div class="flex items-center gap-6">
            ...
            <div x-data="{ open: false }" 
                ...
                <div x-show="open" 
                    ...
                    <div class="bg-gray-50 px-4 py-3 border-b flex justify-between items-center">
                        <h3 class="text-sm font-bold text-gray-800">Persetujuan Masuk</h3>
                        <span class="bg-amber-100 text-amber-700 text-xs font-bold px-2 py-0.5 rounded-full">{{ $jmlNotifUnread }} Antrian</span>
                    </div>

                    <div class="max-h-80 overflow-y-auto">
                        @forelse($notifikasi as $notif)
                            <a wire:click.prevent="markAsRead('{{ $notif->notif_id }}')" href="#" 
                               class="block px-4 py-4 border-b hover:bg-amber-50 transition bg-white">
                                ...
                                <p class="text-[11px] text-gray-500 mt-1">Klik untuk meninjau dan memberi persetujuan.</p>
                            </a>
                        @empty
                            <div class="px-4 py-10 text-center text-gray-500">
                                <p class="text-sm">Tidak ada permintaan menunggu persetujuan.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="h-9 w-9 rounded-full bg-amber-500 flex items-center justify-center text-white font-bold shadow-sm">
