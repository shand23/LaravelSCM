                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($materials as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $item->id_material }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-bold">{{ $item->nama_material }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $item->status_material == 'Aktif' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ...
                                <button wire:click="delete('{{ $item->id_material }}')" wire:confirm="Hapus data material ini?" class="text-red-600 hover:text-red-900">Hapus</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">Belum ada data material.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="p-6">
                <form wire:submit.prevent="store">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Kategori Material <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Nama Material <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="nama_material" placeholder="Contoh: Semen Tiga Roda 50Kg" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm border px-3 py-2">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="satuan" placeholder="Contoh: Sak, Kg, Batang, Zak..." class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm border px-3 py-2">
                            @error('satuan') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Status Material <span class="text-red-500">*</span></label>
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Spesifikasi (Opsional)</label>
                        <textarea wire:model="spesifikasi" rows="2" placeholder="Detail spesifikasi material..." class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm border px-3 py-2"></textarea>
                        @error('spesifikasi') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Standar Kualitas (Opsional)</label>
                        <textarea wire:model="standar_kualitas" rows="2" placeholder="Contoh: SNI, ISO..." class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm border px-3 py-2"></textarea>
                        @error('standar_kualitas') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
